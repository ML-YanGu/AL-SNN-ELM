function [testaccuracy,time,M1,beta1,IW,Bias,labeledS,labeledL]=ELMActive_kmeans_center(labeled,unlabeled,test,ActivationFunction,T,k_value,IW,Bias)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Input:
...
%k_value:The number of the clusters
%Output:
...
%add the representative dataset
T1_index=0;
while T1_index<T1
    %add each cluster center
    [output,center] = kmean(unlabeledS,k_value);
    for i=1:k_value
        data_for_one_class=unlabeledS(output(:,1)==i,:);
        ...
        qIndex = ismember(unlabeledS,data_for_one_class(index(1,1),:), 'rows') == 1;
...
for i=1:T2
   ...
   labeledS(NL1+T1+i,:)=unlabeledS(index,:);  
   labeledL(NL1+T1+i,:)=unlabeledL(index,:); 
   ...
   unlabeledL(index,:)=[];  
   [M,beta,TestingAccuracy]=OSELM(M,beta,incrementS,incrementL,HTest,TV_T,ActivationFunction);
   testaccuracy(1,T1+i+1)=TestingAccuracy;
   time(1,T1+i+1)=cputime;
end
